...
# --- DATA CONSOLIDATION LOGIC (THE "BRAIN" OF THE APP) ---
...
def consolidate_data_by_mobile(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Logic:
    1. Sorts all records by mobile_number, then by date (DESCending).
    2. Groups records by 'mobile_number'.
    3. Checks for a valid 'recording_url' in the group.
    4. If URL exists: Pick the LATEST one for transcription.
    5. If NO URL exists: Concatenate ALL 'outcome' values from the history and skip transcription.
    """
    # 1. Ensure date column is datetime for correct sorting
    if 'date' in df.columns:
        df['date'] = pd.to_datetime(df['date'], errors='coerce')
        # Sort: Mobile (A-Z), Date (Newest to Oldest)
    ...
    for mobile in unique_mobiles:
        # Isolate the group of rows for this specific mobile number
        group = df[df['mobile_number'] == mobile]
        
        # Identify rows that have a valid, non-empty recording URL
        valid_recording_rows = group[group['recording_url'].notna() & (group['recording_url'] != "")]
        
        if not valid_recording_rows.empty:
            # --- CASE A: Audio Exists ---
            # Select the first row (which is the latest date due to sorting)
            selected_row = valid_recording_rows.iloc[0].copy()
            # Mark this row for actual API transcription
            selected_row['processing_action'] = 'TRANSCRIBE'
        else:
            # --- CASE B: No Audio Exists (Fallback) ---
            # We take the absolute latest row to represent the contact
            selected_row = group.iloc[0].copy()
            selected_row['processing_action'] = 'SKIP'
            
            # Logic: Collect ALL outcomes from the history
            # .fillna("") handles NaNs, .astype(str) ensures text
            outcomes = group['outcome'].fillna("").astype(str).tolist()
            
            # Filter out empty strings and join with spaces
            outcome_history = " ".join([x.strip() for x in outcomes if x.strip()])
            
            # Populate the transcript column with this history
    ...
    # Return a new DataFrame containing only the unique rows to be processed/displayed
...
def process_single_row(index: int, row: pd.Series, api_key: str, prompt_template: str, keep_remote: bool = False) -> Dict[str, Any]:
    ...
    result = {
        ...
        "transcript": row.get("transcript", ""), # Might be pre-filled with outcomes
        "status": row.get("status", "Pending"),
        "error": row.get("error", None),
    }

    # Check the flag set by the consolidation logic
...
def main():
    ...
    if start_button:
        ...
        # This function handles the "Latest URL" and "Outcome History" logic
